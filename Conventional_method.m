...
for y=1:10
   ...
   for  z=1:it
      ...
      Great_Grand_Throughput_r=0;
      disp('Iteration:');
      disp(z);% Data generate of 16 ONU
      disp('-------------------------------------------------------------');
      for j=1:16;
          r(j)=2*randi([0 y],1,1);     % random data packets
          W_r=12.576*r+R;              % Requested window size
          Report_Message=round(W_r);
      end
      disp('Value of Report_Message:');
      disp(Report_Message);
...
%                                             % Calculate the number of Exhausted ONUs                                              
    ...
    for j=1:16
        if Report_Message(1,j)>=W_max
            Get_Message(1,j)=W_max;
...
%       %Great_Grand_Throughput_r_Store(1,z) = Great_Grand_Throughput_r;
   ...
   avg=Great_Grand_Throughput_r_Sum/it;
   disp('Value of avg:' );
   disp(avg);
   xp(y)=y;
   yp(y)=avg;
